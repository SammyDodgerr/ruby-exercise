...
1. Bring your solution with you on your own machine to the interview and make sure it's ready to work on (let us know if you need us to provide a laptop). If we're interviewing remotely, please be ready to share your screen.
...
Acme will provide the order updates in batches (twice daily), in the form of a JSON file uploaded to our server. These files vary in size, but often contain tens-of-thousands of orders and are several megabytes in size.
...
The contents of each order object indicates the action Shift needs to take. Our rake task in this project will be called by another system, which will invoke our rake task with the path to the newly uploaded file (see below for an example).
...
1. Fix the failing test
   ...
   We've provided an example file and test to get you started. However, the test is failing due to some missing functionality.

   Update the `ShipmentsProcessor#call` method so that the test passes.
...
2. Ignore orders not relevant to the Shift OMS
   ...
   Update the `ShipmentsProcessor#call` method to return the correct list of orders.

3. Split orders into fulfillments and cancellations

   Of the remaining orders, we need to either create a fulfillment, or cancel the order in the Shift OMS. If all order line items in an order have a `QUANTITY` of 0 (zero) then cancel the order in Shift's OMS, otherwise fulfil the order.

   Update the `ShipmentsProcessor#call` method to return two lists of orders, one to fulfil and another to cancel.
